...
def create_app(config_class=Config):
    ...
    if (
        ...
        and database_uri != "sqlite:///:memory:"
    ):
    ...
    with app.app_context():
        if (
            ...
            or "sqlite:///:memory:"
            in app.config.get("SQLALCHEMY_DATABASE_URI", "")
        ):
            db.create_all()
    ...
    # -------------------------------------------------------------------------
    # IMPORTANT:
    # CSRF tokens are tied to the Flask session.
    #
    # Never allow dynamic HTML pages containing session-specific CSRF tokens
    # to be cached by the browser/CDN.
    # -------------------------------------------------------------------------
    @app.after_request
    ...
    def handle_csrf_error(e):
        ...
        flash(
            "Session expired or invalid CSRF token. Please try again.",
            "danger",
        )
    ...
    def ratelimit_handler(e):
        ...
        flash(
            f"Too many attempts. {description}",
            "danger",
        )
    ...
    def _prepare_idempotency(operation, data):
        ...
        try:
            record, is_new = reserve_idempotency_record(
                current_user.id,
                idem_key,
                operation,
                req_hash,
        ...
        if not is_new:
            if (
                ...
                and record.response_code is not None
            ):
                return None, (
                    jsonify(json.loads(record.response_body)),
    ...
    def merchant_register():
        ...
        if request.method == "POST":
            ...
            while True:
                ...
                if not Merchant.query.filter_by(
            ...
            flash(
                f"Congratulations! Your business "
                f"'{merchant.name}' is registered.",
                "success",
            )
    ...
    def merchant_qr(id):
        ...
        if merchant.status != "ACTIVE":
            return api_error(
                "merchant_inactive",
                f"Merchant '{merchant.name}' is inactive.",
                400,
            )
    ...
    def pay_page(qr_code_id):
        ...
        if merchant.status != "ACTIVE":
            flash(
                f"Merchant '{merchant.name}' is currently "
        ...
        if request.method == "POST":
            ...
            try:
                ...
                flash(
                    f"Payment of {format_npr(tx.amount_paisa)} "
                    f"to {merchant.name} was successful! "
                    f"Ref: {tx.reference}",
                    "success",
                )
    ...
    def api_merchant_dashboard():
        """
        Returns merchant dashboard information for the current user.
        """
        if not current_user.is_merchant:
            return api_error(
                "forbidden",
                "Forbidden. User is not a registered merchant.",
                403,
            )
        ...
        today_txs = merchant_query.filter(
            Transaction.timestamp >= start_of_today
        ).all()

        total_paisa_today = sum(
            tx.amount_paisa for tx in today_txs
        )

        total_received_today = float(
            paisa_to_npr(total_paisa_today)
        )
        ...
        paginated = merchant_query.order_by(
        ...
        return jsonify(
            {
                ...
                "merchants": [
                    m.to_dict()
                    for m in current_user.merchants
                ],
    ...
    def api_transaction_receipt(reference):
        ...
        if not tx:
            return api_error(
                "not_found",
                f"Transaction with reference '{clean_ref}' not found.",
                404,
            )
        ...
        is_participant = bool(
            ...
            and (
                current_user.id
                in (tx.sender_id, tx.receiver_id)
                or (
                    tx.merchant
                    and tx.merchant.owner_user_id
                    == current_user.id
                )
            )
        )
    ...
    def api_send():
        ...
        idem_record, idem_error = _prepare_idempotency(
            "send",
            data,
        )
        ...
        try:
            ...
            return api_error(
                "internal_error",
                "Failed to complete transfer due to an internal error.",
                500,
            )
    ...
    def api_pay():
        ...
        try:
            ...
            return api_error(
                "internal_error",
                "Failed to process payment due to an internal error.",
                500,
            )
    ...
    def api_topup():
        """
        Credit user wallet directly via simulated top-up.
        """
        data = request.get_json(silent=True)
        ...
        try:
            ...
            return api_error(
                "internal_error",
                "Failed to process top-up due to an internal error.",
                500,
            )
    ...
    def signup():
        ...
        if request.method == "POST":
            ...
            try:
                name, clean_phone, password = validate_signup_payload(
                    payload
                )
            ...
            flash(
                f"Namaste {new_user.name}! Your account has been "
                "created with NPR 5,000 welcome balance.",
                "success",
            )
    ...
    def login():
        ...
        if request.method == "POST":
            ...
            if not phone_raw or not password:
                err = (
                    "Please provide both mobile number "
                    "and password."
                )
            ...
            try:
                clean_phone = normalize_nepal_phone(
                    phone_raw
                )
    ...
    # -------------------------------------------------------------------------
    @app.route(
        "/api/admin/transactions/<int:transaction_id>/reverse",
        methods=["POST"],
    )
    @login_required
    ...
    def dashboard():
        ...
        sample_merchants = Merchant.query.filter_by(
            status="ACTIVE"
        ).limit(4).all()

        return render_template(
            "dashboard.html",
            title="My Wallet - Paisa Wallet",
